...
import au.com.fairfaxdigital.common.database.interfaces.OnTaskCompleteListener;
import au.com.fairfaxdigital.common.database.interfaces.Tasker;
...
public class Datastore implements OnTaskCompleteListener {
	...
	public void executeQuery(int token, Query stmt, OnQueryComplete listener) {
		try{
			...
			listener.onQueryComplete(token, null, e);
		}
		...
		task.addOnQueryCompleteListener(listener);
     ...
     * @throws DatabaseException
     */
	public RSData executeQuery(Query stmt) throws DatabaseException {
		if(!validateDBConnection()){
			throw new DatabaseException(
				"Attempt to reopen an already closed database object. "
				+ "Ensure a connection to the database is currently valid and open");
	...
	public <T> List<T> injectData(Query stmt, Class classToInject) throws DatabaseException {
		if(!validateDBConnection()){
			throw new DatabaseException(
				"Attempt to reopen an already closed database object. "
				+ "Ensure a connection to the database is currently valid and open");
		...
		return task.getInjectedObjects(classToInject);
	}
	...
	public void executeNonQuery(int token, Statement stmt,
		...
		try{
			...
			listener.onNonQueryComplete(token, 0, e);
		}
	...
	public void executeAddOrUpdate(int token, final String whereClause, final Insert insert,
		final OnNonQueryComplete listener) {
		Query query = new Query(insert.getTable(), whereClause);
		executeQuery(token, query, new OnQueryComplete() {
			...
			public void onQueryComplete(int token, RSData resultSet, DatabaseException e) {
				...
				if(resultSet.getCount() <= 0){
					...
					List<Map<String, Object>> values = insert.getValues();
					
					if(!values.isEmpty()) {
						Update update = new Update(insert.getTable(),
							values.get(0), whereClause, null);
				...
				if(updateOrAddStmt != null) {
					...
					listener.onNonQueryComplete(token, 0, new DatabaseException(
