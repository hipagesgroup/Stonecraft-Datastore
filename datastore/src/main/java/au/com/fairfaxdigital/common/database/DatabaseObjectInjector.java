...
public class DatabaseObjectInjector {
	 ...
	 */
	public <T> List<T> inject(RSData data, Class<T> classOfT) throws DatabaseException{
		List<T> returnClasses = new ArrayList<T>();
		
		data.moveToFirst();
		while(!data.isAfterLast()) {
			T injectedRow = getInjectedClass(data, classOfT);
			if(injectedRow != null) {
				returnClasses.add(injectedRow);
			}
			data.next();
		}
		...
		try {
			...
			for(Field field : fields) {
				...
				if(annotation instanceof InjectData){
					...
					InjectData InjectAnnotation = (InjectData) annotation;
